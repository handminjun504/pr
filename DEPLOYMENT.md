...
프론트는 **정적 파일**, API는 **PocketBase** 입니다. Vercel·Supabase는 사용하지 않습니다.
...
- Nginx, Caddy, gl의 정적 경로 등 아무 정적 호스팅이면 됩니다.
...
## 4. 레거시 Supabase → GL (선택)

과거 Supabase 덤프가 있을 때만:

```bash
npm run migrate:legacy-supabase-to-gl
```

`.env.migrate` 및 [gl-migration/MIGRATION.md](gl-migration/MIGRATION.md) 참고. 키·덤프 파일은 저장소에 올리지 마세요.
...
과거 Supabase용 SQL 참고용입니다. 운영 스키마는 **gl-migration** 의 PocketBase 스키마가 기준입니다.
